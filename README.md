...
> This information is used to create the *ormconfig.json* file and can be modified after it has been generated.
...
The **Secret Key** must exists (either provided by the user or generated by the CLI) because the Example template has ***Authentication and Authorization*** examples included, if you don't intend to use ***Authentication*** in your GraphQL API you won't need a **Secret Key**.
...
***ngrok*** is a tool that will create a reverse proxy to your API, to make it accessible remotely allowing you to conveniently access your API from outside your local machile or lan network, it is very usefull when testing your API with a mobile client or from a remote connection, we recommend to select ***yes*** and allow the CLI to configure ngrok for you.
